import styled from "styled-components"

export const Body = styled.div`
background-color: #800000;
height: 100vh;
`

export const Header = styled.div`
display: flex;
justify-content: space-between;
border: solid 1px;
background-color: #8FBC8F;
`

export const Main = styled.div`
display: flex;
margin-top: 180px;
flex-direction: column;
justify-content: center;
align-items: center;
`

export const Form = styled.div`
display: flex;
margin-top: 131px;
flex-direction: column;
justify-content: center;
align-items: center;
`

export const SubscribeButton = styled.button`
width: 500px;
height: 200px;
background-color: black;
color: white;
border-radius: 20px;
`

export const FormButton = styled.button `
margin-top: 5px;
border-radius: 200px;
background-color: black;
color: white;
`

export const Login = styled.div`
display: flex;
flex-direction: column;
justify-content: center;
align-items: center;
margin-top: 200px;
`

export const ReturnButton = styled.button`
width: 200px;
height: 40px;
margin-right: 80px;
margin-top: 25px;
background-color: black;
color: white;
border-radius: 200px;
`

export const LabeX = styled.h2`
margin-left: 15px;
font-size: 30px;

`

export const Footer = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 161px;
`
export const FooterLabel = styled.p`
margin-right: 10px;
font-style: italic;

`

export const TextBox = styled.input`
height: 80px;
width: 400px;
`

export const FooterForm = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 96px;
`

export const FooterLogin = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 219px;
`

export const AdminHome = styled.div`
display: flex;
flex-direction: column;
justify-content: center;
align-items: center;
margin-top: 100px;
text-align: center;
`

export const HomeAdminButtons = styled.div`
display: flex;
justify-content: center;
`

export const AdminButtons = styled.button`
padding: 10px;
margin: 40px;
width: 200px;
height: 200px;
background-color: black;
color: white;
border-radius: 200px;
`

export const FooterAdmin = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 113px;
`

export const CreateTrips = styled.div`
display: flex;
flex-direction: column;
justify-content: center;
align-items: center;
`

export const FooterCreate = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 241px;
`

export const StyledH2Div = styled.div`
display: flex;
justify-content: center;
align-items: center;
`

export const FooterTrips = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 218px;
`

export const StyledPositiveSpan = styled.span`
color: #FFD700;
`

export const StyledNegativeSpan = styled.span`
color: white;
`

export const FooterApproval = styled.div`
display: flex;
justify-content: flex-end;
background-color: #8FBC8F;
bottom: 0;
margin-top: 65px;
`




